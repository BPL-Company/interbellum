...
class BotUtil(TeleBot):
    ...
    def mute(self, chat, user, admin_user, for_date=0, reason=""):
        ...
        tts = 'Пользователь {} ограничен на {} {}.'.format(self.get_link(user.first_name, user.id), time_text,
                                                           date_text)
        if for_date < 60:
    ...
    def ban(self, chat, user, admin_user, for_date=0, reason=""):
        ...
        if 60 < for_date < 3600:
            ...
            date_text = "дней"
            time_text = str(for_date / 86400)
        tts = 'Пользователь {} заблокирован на {} {}.'.format(self.get_link(user.first_name, user.id), time_text,
                                                              date_text)
        if for_date < 60:
